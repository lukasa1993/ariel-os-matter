...
device_type!(DoorLockController, 0x000B, 1, "Door Lock Controller", LocksClosures);
...
device_type!(SecondaryNetworkInterface, 0x0019, 1, "Secondary Network Interface", Foundation);
...
device_type!(WaterFreezeDetector, 0x0041, 1, "Water Freeze Detector", Water);
...
device_type!(TemperatureControlledCabinet, 0x0071, 1, "Temperature Controlled Cabinet", Appliances);
device_type!(RoomAirConditioner, 0x0072, 1, "Room Air Conditioner", Appliances);
...
device_type!(NetworkInfrastructureManager, 0x0090, 1, "Network Infrastructure Manager", ThreadInfrastructure);
device_type!(ThreadBorderRouter, 0x0091, 1, "Thread Border Router", ThreadInfrastructure);
...
device_type!(ColorDimmerSwitch, 0x0105, 3, "Color Dimmer Switch", Switches);
...
device_type!(DimmablePluginUnit, 0x010B, 3, "Dimmable Plug-in Unit", Lighting);
device_type!(ColorTemperatureLight, 0x010C, 3, "Color Temperature Light", Lighting);
device_type!(ExtendedColorLight, 0x010D, 4, "Extended Color Light", Lighting);
device_type!(MountedOnOffControl, 0x010F, 1, "Mounted On/Off Control", Switches);
device_type!(MountedDimmableLoadControl, 0x0110, 1, "Mounted Dimmable Load Control", Switches);
device_type!(JointFabricAdministrator, 0x0130, 1, "Joint Fabric Administrator", Foundation);
...
device_type!(WindowCoveringController, 0x0203, 2, "Window Covering Controller", LocksClosures);
device_type!(Closure, 0x0230, 1, "Closure", LocksClosures);
device_type!(ClosurePanel, 0x0231, 1, "Closure Panel", LocksClosures);
device_type!(ClosureController, 0x023E, 1, "Closure Controller", LocksClosures);
...
device_type!(ThermostatController, 0x030A, 1, "Thermostat Controller", Hvac);
device_type!(EnergyEvse, 0x050C, 2, "Energy EVSE", Energy);
device_type!(DeviceEnergyManagement, 0x050D, 2, "Device Energy Management", Energy);
device_type!(WaterHeater, 0x050F, 1, "Water Heater", Energy);
device_type!(ElectricalSensor, 0x0510, 2, "Electrical Sensor", Energy);
device_type!(ElectricalUtilityMeter, 0x0511, 1, "Electrical Utility Meter", Energy);
device_type!(MeterReferencePoint, 0x0512, 1, "Meter Reference Point", Energy);
device_type!(ElectricalEnergyTariff, 0x0513, 1, "Electrical Energy Tariff", Energy);
    ...
    fn enabled_catalog_has_no_duplicate_or_excluded_id() {
        ...
        if let Ok(catalog) = catalog {
            for (index, device) in catalog.iter().enumerate() {
                assert!(!EXCLUDED_DEVICE_TYPES.iter().any(|excluded| excluded.id == device.id));
                assert!(!catalog.iter().skip(index + 1).any(|other| other.id == device.id));
            }
